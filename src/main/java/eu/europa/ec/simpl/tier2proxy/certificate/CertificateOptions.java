...
public record CertificateOptions(
	...
	public record PrivateKey(
			String algo, AlgorithmParameterSpec algoParameterSpec,
			SecureRandom random
	) {
	...
	public record CertificateValidity(
			int beforeDateAmountToSubtract, int afterDateAmountToAdd,
	    @NonNull ChronoUnit beforeDateAmountToSubtractUnit, @NonNull ChronoUnit afterDateAmountToAddUnit
	) {
		ValidityPeriod of() {
			return new ValidityPeriod(beforeDateAmountToSubtract, afterDateAmountToAdd, beforeDateAmountToSubtractUnit, afterDateAmountToAddUnit);
		}
	}
